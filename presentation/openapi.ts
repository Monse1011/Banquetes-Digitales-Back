    ...
    schemas: {
      CreateReservationRequest: {
        ...
        properties: {
          ...
          event_date_time: { type: 'string', format: 'date-time', example: '2026-11-20T19:00:00Z' },
          guest_count: { type: 'integer', minimum: 1, example: 120 },
          event_address: { type: 'string', example: 'Colonia Chuburna C.45' },
          services_ids: { type: 'array', items: { type: 'integer', example: 1 }, example: [1, 2, 4] }
        }
      },
      ReservationCreated: {
        type: 'object',
      ...
      RequestSummary: {
        ...
        properties: {
          ...
          client_email: { type: 'string', format: 'email', example: 'andrea@gmail.com' },
      ...
      RequestList: {
        type: 'object',
        properties: {
          data: { type: 'array', items: { $ref: '#/components/schemas/RequestSummary' } },
          pagination: {
            type: 'object',
            properties: {
      ...
      ApprovalRequest: {
        ...
        required: ['status'],
        properties: {
          status: { type: 'string', enum: ['Aprobado', 'Aprobada'], example: 'Aprobado' }
        }
      },
      ApprovalResponse: {
        type: 'object',
  ...
  paths: {
    '/api/client/request': {
      post: {
        ...
        responses: {
          '201': { description: 'Solicitud creada', content: { 'application/json': { schema: { $ref: '#/components/schemas/ReservationCreated' } } } },
    ...
    '/api/admin/requests': {
      get: {
        ...
        responses: {
          '200': { description: 'Solicitudes paginadas', content: { 'application/json': { schema: { $ref: '#/components/schemas/RequestList' } } } },
          '401': { description: 'Token ausente o invalido', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          '403': { description: 'El usuario no es administrador', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } }
        }
      }
    },
    '/api/admin/requests/{id}': {
      get: {
        ...
        responses: {
          '200': { description: 'Detalle de la solicitud', content: { 'application/json': { schema: { $ref: '#/components/schemas/RequestDetail' } } } },
      ...
      patch: {
        ...
        responses: {
          '200': { description: 'Solicitud aprobada', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApprovalResponse' } } } },
          '400': { description: 'Estado invalido', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' } } } },
          '401': { description: 'Token ausente o invalido' },
